...
int main(int argc, char* argv[]){
    ...
    auto scan = [tree, num_data, num_threads, ops](uint64_t tid, bool){
	size_t chunk = num_data / num_threads;
    ...
    auto read = [tree, num_data, num_threads, ops](uint64_t tid, bool){
	size_t chunk = num_data / num_threads;
	size_t start = chunk * tid;
	size_t end = chunk * (tid+1);
	if(end > num_data)
    ...
    auto mix = [tree, num_data, num_threads, ops](uint64_t tid, bool){
	size_t chunk = num_data / num_threads;
	size_t start = chunk * tid;
	size_t end = chunk * (tid+1);
	if(end > num_data)
