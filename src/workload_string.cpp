...
typedef GenericKey<32> keytype;
typedef GenericComparator<32> keycomp;

extern bool hyperthreading;

using KeyEuqalityChecker = GenericEqualityChecker<32>;
using KeyHashFunc = GenericHasher<32>;
...
// Whether to measure bandwidth
static bool memory_bandwidth = false;
// Skew factor of the workload
...
inline void load(std::string input, int num_thread, int wl, int kt, int index_type, kvpair_t<keytype>* init_kv, int init_num, kvpair_t<keytype>* run_kv, int& run_num, int* ranges, int* ops, std::vector<kvpair_ts_t<keytype>>* init_ts_kv, std::vector<kvpair_ts_t<keytype>>* run_ts_kv) {
	...
	skew << skew_factor;
	if(wl == WORKLOAD_A){
	    init_file.append("loada_int_100M_").append(skew.str()).append(".dat");
	    txn_file.append("txnsa_int_100M_").append(skew.str()).append(".dat");
	}
	else if(wl == WORKLOAD_B){
	    init_file.append("loadb_int_100M_").append(skew.str()).append(".dat");
	    txn_file.append("txnsb_int_100M_").append(skew.str()).append(".dat");
	}
	else if(wl == WORKLOAD_C){
	    init_file.append("loadc_int_100M_").append(skew.str()).append(".dat");
	    txn_file.append("txnsc_int_100M_").append(skew.str()).append(".dat");
	}
	else if(wl == WORKLOAD_E){
	    init_file.append("loade_int_100M_").append(skew.str()).append(".dat");
	    txn_file.append("txnse_int_100M_").append(skew.str()).append(".dat");
	}
	else if(wl == WORKLOAD_MIXED){
	    init_file.append("load_mixed_int_100M_").append(skew.str()).append(".dat");
	    txn_file.append("txns_mixed_int_100M_").append(skew.str()).append(".dat");
	}
	else{
    ...
    else if(kt == EMAIL_KEY){
	init_file.append("load_email_100M.dat");
	if(wl == WORKLOAD_A)
	    txn_file.append("txnsa_email_100M.dat");
	else if(wl == WORKLOAD_B)
	    txn_file.append("txnsb_email_100M.dat");
	else if(wl == WORKLOAD_C)
	    txn_file.append("txnsc_email_100M.dat");
	else if(wl == WORKLOAD_E)
	    txn_file.append("txnse_email_100M.dat");
	else if(wl == WORKLOAD_MIXED)
	    txn_file.append("txns_mixed_email_100M.dat");
	else{
	    if(wl != WORKLOAD_LOAD){
	        std::cout << "Invalid workload type" << std::endl;
	        exit(0);
...
inline void exec(int wl, int index_type, int num_thread, kvpair_t<keytype>* init_kv, int init_num, kvpair_t<keytype>* run_kv, int run_num, int* ranges, int* ops){
    ...
    auto func_earliest = [idx, init_kv, init_num, num_thread, &earliest_finished, &inserted_num, &local_load_latency, &params, &perf_block, &breakdown](uint64_t thread_id, bool) {
    ...
    auto func = [idx, init_kv, init_num, num_thread, &local_load_latency, &params, &perf_block, &breakdown](uint64_t thread_id, bool) {
	auto random_bool = std::bind(std::bernoulli_distribution(sampling_rate), std::knuth_b());
	size_t total_num_key = init_num;
	size_t key_per_thread = total_num_key / num_thread;
	size_t start_index = key_per_thread * thread_id;
    ...
    auto func2_earliest = [num_thread, idx, &earliest_finished, &op_num, &run_kv, run_num, &ranges, &ops, &local_run_latency, &params, &perf_block, &breakdown](uint64_t thread_id, bool) {
	auto random_bool = std::bind(std::bernoulli_distribution(sampling_rate), std::knuth_b());
...
int main(int argc, char *argv[]) {
	...
	options.add_options()
	    ...
	    ("key_type", "Key type (rand, email)", cxxopts::value<std::string>())
